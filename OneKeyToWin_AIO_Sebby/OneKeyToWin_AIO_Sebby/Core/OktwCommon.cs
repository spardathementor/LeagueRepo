    ...
    class OktwCommon
    {
        ...
        public static bool CanMove(Obj_AI_Hero target)
        {
            if (target.HasBuffOfType(BuffType.Stun) || target.HasBuffOfType(BuffType.Snare) || target.HasBuffOfType(BuffType.Knockup) ||
                ...
                target.IsStunned || (target.IsChannelingImportantSpell() && !target.IsMoving) || target.MoveSpeed < 10 || Prediction.GetPrediction(target,0.25f).Hitchance > HitChance.VeryHigh)
            {
                return false;
            }
            else
