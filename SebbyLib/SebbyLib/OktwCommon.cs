...
namespace SebbyLib
{
    public class OktwCommon
    {
        ...
        {
            if (target.MoveSpeed < 50 || ((!target.CanMove || target.IsRooted || Player.IsImmovable) && !target.IsWindingUp) || target.IsStunned || target.HasBuffOfType(BuffType.Stun) || target.HasBuffOfType(BuffType.Fear) || target.HasBuffOfType(BuffType.Snare) || target.HasBuffOfType(BuffType.Knockup) || target.HasBuff("Recall") ||
